...
public class TestMongoDB {
	...
	public void testLoad() throws Exception {
		...
		MongoCursor<Document> cursor = col.find().filter(filter)
				.projection(fields(include("c_custkey", "c_name", "c_acctbal", "c_comment"), excludeId())).iterator();
		...
		cursor = col.find().filter(filter)
				.projection(fields(include("o_orderkey", "o_custkey", "o_totalprice", "o_clerk"), excludeId()))
				.iterator();
	...
	public void testLoadNest() throws Exception {
		...
		MongoCursor<Document> cursor = col.find().filter(filter)
				.projection(fields(include("c_custkey", "c_name", "c_acctbal", "orders"), excludeId())).iterator();
